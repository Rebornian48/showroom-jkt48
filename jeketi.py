...
if search_item(list_members, x):
    ...
    if res['is_onlive']==True:
        ...
        plt.title("Penonton " + str(res['room_name']) + "\n" + "Data per : " + waktu, loc = 'center')
        plt.xlabel("Poin")
        plt.barh(x,y)
...
if x=="live":
    num=0
    for y in list_id:
        api_url="https://www.showroom-live.com/api/room/profile?room_id=" + str(y)
        response = requests.get(api_url)
        res = response.json()
        stat="";
        if res["is_onlive"]==True:
            ...
            stat="Offline"
        print("Status " + res['room_name'] + " : " + stat)
    print("Jumlah member yang live : " + str(num))
    print("Data jumlah member yang live beserta daftar namanya akan segera hadir")